...
class Training(object):
    def __init__(self, checkpoint, gpu, batch_size):
        ...
        # Model encoder: CNN + MLP
        self.encode_models = encoder_decoder.Encoder(in_channels=6*11, 
                channels_latent=64,
                activation='gelu').to(self.device)
        self.project_models = nn.Linear(64*8, self.config['latent_dim']).to(self.device)
        
        # Stokes encoder: CNN + MLP
        self.encode_stokes = encoder_decoder.Encoder(in_channels=4*112, 
                channels_latent=64,
                activation='gelu').to(self.device)
        self.project_stokes = nn.Linear(64*8, self.config['latent_dim']).to(self.device)

        print("Setting weights of the model...")        
        self.encode_models.load_state_dict(chk['encode_models_dict'])
        self.project_models.load_state_dict(chk['project_models_dict'])
        self.encode_stokes.load_state_dict(chk['encode_stokes_dict'])
        self.project_stokes.load_state_dict(chk['project_stokes_dict'])

        self.stats = chk['stats']
        
        self.encode_stokes.eval()
        self.project_stokes.eval()
        self.encode_models.eval()
        self.project_models.eval()
    ...
    def test(self):
        ...
        self.training_dataset = datasets.Dataset2D(self.config['training_set'], 
                    pctx=[0,80], 
                    pcty=[0,100], 
                    stats=None, 
                    step=5)                
