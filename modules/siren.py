...
class SirenGenerator(nn.Module):
    def __init__(self, npix, dim_latent, dim_hidden, siren_num_layers = 6, mapping_depth = 3):
        super().__init__()

        self.hyperparams = {'npix': npix, 'dim_latent': dim_latent, 'dim_hidden': dim_hidden, 'siren_num_layers': siren_num_layers, 'mapping_depth': mapping_depth}
        self.npix = npix
    ...
    def forward(self, latent):
        ...
        coors = self.coors.unsqueeze(0).expand(batchsize, self.npix, 1)

        # Conditioning parameters that enter into the SIREN
        gamma, beta = self.mapping(latent)

        # SIREN network with the FiLM per-layer conditioning
        x = self.siren(coors, gamma, beta)

        # Final output
        output = self.to_output(x).squeeze()
